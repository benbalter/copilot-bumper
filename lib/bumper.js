      ...
      try {
        ...
        if (isCopilot && stalledStatus) {
          ...
          if (!isFixed && commentsCount < MAX_COMMENTS_PER_RUN) {
            // Use different message if there are merge conflicts
            const commentBody = mergeConflict
              ? '@copilot There is a merge conflict with the base branch. Please merge in the base branch and resolve the conflicts.'
