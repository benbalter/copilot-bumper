...
export function findLatestReviewComment(reviewComments) {
  ...
  // Find the first review comment not from copilot[bot]
  return reviewComments.find(comment => comment.user?.login !== 'copilot[bot]');
}
      ...
      try {
        ...
        if (isCopilot && stalledStatus) {
          ...
          if (!isFixed && commentsCount < MAX_COMMENTS_PER_RUN) {
            // Use different comment based on whether there's line-level feedback
            const commentBody = hasFeedback 
              ? '@copilot please implement the feedback left on this PR.'
