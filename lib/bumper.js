    ...
    async function processNotification(notification) {
      ...
      try {
        ...
        if (isCopilot && stalledStatus) {
          ...
          // Check PR timeline for Copilot session failures
          ...
          try {
            ...
            hasSessionFailure = hasFailedCopilotSession(timelineEvents);
          ...
          // Require WIP unless Copilot had an error, session failed, or checks are failing
          if (!wipStatus && !hasError && !hasSessionFailure && !hasFailingChecks) {
            console.log(`PR ${owner}/${repo}#${prNumber} requires WIP status, Copilot error, session failure, or failing checks to be bumped.`);
            console.log(`isCopilotPr: ${isCopilot}, isWip: ${wipStatus}, isStalled: ${stalledStatus}, hasError: ${hasError}, hasSessionFailure: ${hasSessionFailure}, hasFailingChecks: ${hasFailingChecks}`);
            return;
          }
