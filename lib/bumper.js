 ...
 */
export function isStalled(pr, thresholdMs = 60 * 60 * 1000) {
    ...
    async function processNotification(notification, stallThresholdMs = STALL_THRESHOLD_MS) {
      ...
      try {
        ...
        if (isCopilot && stalledStatus) {
          ...
          // Don't mark as processed if it's just not stalled yet - it might become stalled in second pass
          if (!isCopilot || !isWip(pr)) {
            // If it's not a Copilot PR or not WIP, it won't ever be bumped
            processedPrKeys.add(prKey);
          }
          return false;
        }
